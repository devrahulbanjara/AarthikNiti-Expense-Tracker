  ...
  return (
    <div className={`fixed top-0 left-0 w-1/5 h-full ${darkMode ? "bg-gray-900" : "bg-white"} p-4 border-r ${darkMode ? "border-gray-700" : "border-gray-200"} min-h-screen z-30 transition-colors duration-300 flex flex-col`}>
      ...
      <div className="mb-4 relative account-dropdown-container">
        ...
        {showAccountDropdown && (
          <div className={dropdownClasses}>
            ...
            <div className={`p-2 border-t ${darkMode ? "border-gray-700 hover:bg-gray-700" : "border-gray-200 hover:bg-gray-100"} cursor-pointer text-blue-500`}>
              + Add Account
            </div>
          </div>
        )}
      ...
      <ul>
        {navItems.map((item) => (
          <li key={item.name}>
            <Link to={item.href} className={`${navLinkClasses} ${item.name === "Dashboard" ? (darkMode ? "bg-gray-800" : "bg-gray-200") : ""}`}>

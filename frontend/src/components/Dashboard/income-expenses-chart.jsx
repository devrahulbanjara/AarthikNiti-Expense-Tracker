  ...
  const [loading, setLoading] = useState(true)
    ...
    const CustomTooltip = ({ active, payload, label }) => {
        if (active && payload && payload.length) {
            return (
                <div className={`${darkMode ? "bg-gray-700 border-gray-600" : "bg-white border-gray-200"} p-3 border rounded-md shadow-md`}>
                    ...
                    <p className="text-sm">
                        <span className="font-medium text-blue-500">Savings:</span> ${ (payload[0].value - payload[1].value).toLocaleString()}
                    </p>
                </div>
            );
        }
    ...
    return (
        <div className={`${darkMode ? "bg-gray-800" : "bg-white"} p-4 rounded-lg border ${darkMode ? "border-gray-700" : "border-gray-300"} w-full -ml-10`}>
            <div className="flex justify-between items-start mb-2">
                ...
                <div className="relative">
                    <button
                        className={`flex items-center gap-1 px-3 py-2 border rounded-md text-sm cursor-pointer ${darkMode ? "bg-gray-700 border-gray-600 hover:bg-gray-600" : "bg-white border-gray-300 hover:bg-gray-50"}`}
                        onClick={() => setShowTimeDropdown(!showTimeDropdown)}
                    >
                        {timeRange} <ChevronDown className="h-4 w-4" />
                    </button>
                    {showTimeDropdown && (
                        <div className={`absolute right-0 mt-1 w-40 ${darkMode ? "bg-gray-700 border-gray-600" : "bg-white border-gray-300"} border rounded-md shadow-lg z-10`}>
                            ...
                            <div className="grid grid-cols-3 gap-2 mt-1">
                                ...
                                <div className="text-center">
                                    ...
                                    <p className="font-medium text-blue-500">
                                        ${(hoveredBar.income - hoveredBar.expenses).toLocaleString()}
                                    </p>
                                </div>
                            </div>
                        </div>

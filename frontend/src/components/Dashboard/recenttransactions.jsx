...
import { Search, Filter, Edit, Trash, ArrowUp, ArrowDown, Plus, X } from "lucide-react";
...
const RecentTransactions = ({ onTransactionsChange }) => {
  ...
  const filteredTransactions = transactions.filter((t) => {
    ...
    // Adjust fromDate to start of day
    if (fromDate) {
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row gap-2 mb-4">
        ...
        <button
          ...
          className={`px-4 py-2 rounded-lg text-sm flex items-center justify-center gap-1 transition-all duration-200 
            ${darkMode 
              ? "bg-gray-700 hover:bg-gray-600 text-white" 
              : "bg-gray-100 hover:bg-gray-200 text-gray-700"}
            ${showFilters ? (darkMode ? 'bg-gray-600' : 'bg-gray-200') : ''}`}
      ...
      {showFilters && (
        <div
          className={`mb-4 p-4 border rounded-lg transition-all duration-300 animate-slideDown 
            ${darkMode 
        ...
        >
          <div className="flex justify-between items-center mb-3">
            ...
            <button
              ...
              className="text-xs text-blue-500 hover:text-blue-400 hover:underline transition-colors duration-200 flex items-center gap-1"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium mb-1 ${darkMode ? 'text-gray-300' : 'text-gray-600'}">Category</label>
              <select
                name="category"
                value={filters.category}
                onChange={handleFilterChange}
                className={`w-full p-2 border rounded-lg text-sm transition-colors duration-200 
                  ${darkMode 
                    ? "bg-gray-600 border-gray-500 text-white focus:ring-green-500 focus:border-green-500" 
            ...
            <div>
              <label className="block text-xs font-medium mb-1 ${darkMode ? 'text-gray-300' : 'text-gray-600'}">Description</label>
              <input
                type="text"
                name="description"
                ...
                className={`w-full p-2 border rounded-lg text-sm transition-colors duration-200 
                  ${darkMode 
                    ? "bg-gray-600 border-gray-500 text-white placeholder-gray-400 focus:ring-green-500 focus:border-green-500" 
                    : "bg-white border-gray-300 text-gray-800 placeholder-gray-500 focus:ring-green-500 focus:border-green-500"}
                  focus:outline-none focus:ring-2`}
              />
            </div>
            <div>
              <label className="block text-xs font-medium mb-1 ${darkMode ? 'text-gray-300' : 'text-gray-600'}">Date From</label>
              <input
                type="date"
                name="dateFrom"
            ...
            <div>
              <label className="block text-xs font-medium mb-1 ${darkMode ? 'text-gray-300' : 'text-gray-600'}">Date To</label>
              <input
                type="date"
                name="dateTo"
